  ...
  return (
    <div data-testid="dashboard-shell" className="mx-auto max-w-[1200px] space-y-6 p-4 md:p-8">
      <DashboardOverview
        ...
        nextAction={{
          title: nextAction?.title ?? "Keep the logging loop alive",
          body:
            nextAction?.body ??
            "Log one more real expense so EVA can keep your dashboard, statements, and insights aligned.",
          ctaLabel: nextAction?.cta_label ?? "Open AI Advisor",
        }}
        onOpenNextAction={() => navigate(nextAction?.cta_href ?? "/chat")}
      />

      <DashboardStatsGrid stats={stats} fadeUp={fadeUp} />
